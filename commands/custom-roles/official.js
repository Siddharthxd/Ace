...
async function handleOfficialCommand(client, message, args) {
  ...
  if (!args[0]) {
    const embed = new MessageEmbed()
      .setColor(client.color)
      .setDescription(`Usage: ${prefix}official <user>`);
  ...
  const officialRole = await getRole(client, `official_${message.guild.id}`);
  if (!officialRole) {
    await client.db3.set(`official_${message.guild.id}`, null);

    const embed = new MessageEmbed()
      .setColor(client.color)
      .setDescription(`<a:axery_Cross:1243171146480881735> | **Official Role** not found.`);

    return message.channel.send({ embeds: [embed] });
  }

  if (!message.guild.roles.cache.has(officialRole)) {
    await client.db3.set(`official_${message.guild.id}`, null);
  ...
  const hasOfficialRole = mentionedMember.roles.cache.has(officialRole);
  if (hasOfficialRole) {
    mentionedMember.roles.remove(officialRole);
    embed.setDescription(`<a:axery_Tick:1243171077954207745> | Successfully removed <@&${officialRole}> from ${mentionedMember}`);
  } else {
    mentionedMember.roles.add(officialRole);
    embed.setDescription(`<a:axery_Tick:1243171077954207745> | Successfully added <@&${officialRole}> to ${mentionedMember}`);
  }
...
module.exports = {
  ...
  run: async (client, message, args) => {
    handleOfficialCommand(client, message, args);
  },
};
