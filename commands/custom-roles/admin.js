...
async function handleAdminCommand(client, message, args) {
  ...
  if (!extraOwner.includes(message.author.id) && !extraAdmin.includes(message.author.id) && !message.guild.ownerId.includes(message.author.id) && !message.member.permissions.has('ADMINISTRATOR') && !ownerIDS.includes(message.author.id) && !message.member.roles.cache.has(requiredRole)) {
    const embed = new MessageEmbed()
      ...
      .setDescription(`<a:axery_Cross:1243171146480881735> | You need to be either the Server Owner, Admin or have the Required Role to execute this command.`)
  ...
  if (!mentionedMember) {
    return message.channel.send({ content: `<a:axery_Cross:1243171146480881735> | Please provide a valid user.` });
  }
  ...
  if (!adminRole) {
    ...
    const embed = new MessageEmbed()
      .setColor(client.color)
      .setDescription(`<a:axery_Cross:1243171146480881735> | **Admin Role** not found.`);
  ...
  if (rolePermissions.some(permission => dangerousPermissions.includes(permission))) {
    ...
    const embed = new MessageEmbed()
      .setColor(client.color)
      .setAuthor(client.user.tag, client.user.displayAvatarURL())
      .setDescription(`<a:axery_Cross:1243171146480881735> | The mentioned role cannot be added because it contains the following dangerous permissions: ${permissionsString}.`)
      .setFooter(client.user.tag, client.user.displayAvatarURL());
  ...
  if (hasAdminRole) {
    ...
    embed.setDescription(`<a:axery_Tick:1243171077954207745> | Successfully removed <@&${adminRole}> from ${mentionedMember}`);
  } else {
    mentionedMember.roles.add(adminRole);
    embed.setDescription(`<a:axery_Tick:1243171077954207745> | Successfully added <@&${adminRole}> to ${mentionedMember}`);
  }
