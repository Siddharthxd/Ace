...
async function handleGuestCommand(client, message, args) {
  ...
  if (!guestRole) {
    ...
    const embed = new MessageEmbed()
      .setColor(client.color)
      .setDescription(`<a:axery_Cross:1243171146480881735> | **Guest Role** not found.`);
  ...
  if (hasGuestRole) {
    ...
    embed.setDescription(`<a:axery_Tick:1243171077954207745> | Successfully removed <@&${guestRole}> from ${mentionedMember}`);
  } else {
    mentionedMember.roles.add(guestRole);
    embed.setDescription(`<a:axery_Tick:1243171077954207745> | Successfully added <@&${guestRole}> to ${mentionedMember}`);
  }
