...
async function handleGirlCommand(client, message, args) {
  ...
  if (!girlRole) {
    ...
    const embed = new MessageEmbed()
      .setColor(client.color)
      .setDescription(`<a:axery_Cross:1243171146480881735> | **Girl Role** not found.`);
  ...
  if (hasGirlRole) {
    ...
    embed.setDescription(`<a:axery_Tick:1243171077954207745> | Successfully removed <@&${girlRole}> from ${mentionedMember}`);
  } else {
    mentionedMember.roles.add(girlRole);
    embed.setDescription(`<a:axery_Tick:1243171077954207745> | Successfully added <@&${girlRole}> to ${mentionedMember}`);
  }
