...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		const guide = new MessageEmbed()
			...
			.addField(
				`<:axery_arrow:1243170991266332703> ${prefix}bl add <user> all`,
				`Add a user to blacklisted users for all servers.`
			)
			.addField(
				`<:axery_arrow:1243170991266332703> ${prefix}bl remove <user> all`,
				`Remove a user from blacklisted users from all servers.`
			)
			.addField(
				`<:axery_arrow:1243170991266332703> ${prefix}bl show`,
				`Shows all the users in blacklisted database.`
			)
			.addField(
				`<:axery_arrow:1243170991266332703> ${prefix}bl reset`,
				`Removes all the users from blacklisted users from the database.`
			)
			.setFooter(
				`Made by Ace Development`,
