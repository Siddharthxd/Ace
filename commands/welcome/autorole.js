...
async function displayConfig(message, data, client, arypton) {
    ...
    function generateEmbed(page) {
        ...
        currentHumanRoles.forEach((roleId, i) => {
            ...
            if (role) {
                roleMention.push(`\`[${startIndex + i + 1}]\` | ID: [${roleId}](https://discord.gg/eroshq) | <@&${roleId}>`);
            } else {
                roleMention.push(`\`[${startIndex + i + 1}]\` | ID: [${roleId}](https://discord.gg/eroshq) | Deleted Role`);
            }
        });
        ...
        currentBotRoles.forEach((roleId, i) => {
            const role = message.guild.roles.cache.get(roleId);
            if (role) {
                roleMention1.push(`\`[${startIndex + i + 1}]\` | ID: [${roleId}](https://discord.gg/eroshq) | <@&${roleId}>`);
            } else {
                roleMention1.push(`\`[${startIndex + i + 1}]\` | ID: [${roleId}](https://discord.gg/eroshq) | Deleted Role`);
            }
        });
        ...
        wlistembed.setFooter(`Made by Ace Development`, `${arypton.displayAvatarURL({ dynamic: true })}`);
