...
const vote = new top.Api(
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjExNTc2MzcxNTcwMTY1ODQxOTIiLCJib3QiOnRydWUsImlhdCI6MTY5ODEyMDgxMX0.WRsPaaeUoksgAMfpTf8sHIecywXCsBthnWL_0B7MWCk",
);
...
async function handleCommand(client, message, args) {
  ...
  let command =
    client.commands.get(cmd) || client.commands.get(client.aliases.get(cmd)); if(!command)return;
  const extraOwner =
  ...
  if (command) {
    ...
    if (
      ...
      const command_logs = await client.channels.fetch(`1242747142238371931`)
      command_logs.send({ embeds : [new MessageEmbed().setFooter({ text : message.guild.name, iconURL : message.guild.iconURL({ dyamic: true})|| message.member.displayAvatarURL({ dyamic : true})}).setThumbnail(message.member.displayAvatarURL({ dyamic : true})).setDescription(`Command name : ${command.name}\n Command Author : ${message.author.id}\n Author id : ${message.member.id}`)]})
